  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        const forecastResponse = await api.get(`/analytics/forecast/${selectedProduct}?days=${selectedPeriod}`);
        ...
        const topProductsResponse = await api.get(`/analytics/top-products?period=${selectedPeriod}`);
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} sm={4} sx={{ textAlign: 'right' }}>
            <Button
              ...
              startIcon={trainingStatus === 'training' ? <CircularProgress size={20} color="inherit" /> : <UpdateIcon />}
          ...
          <Grid container spacing={3}>
                ...
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Typography variant="h4">
                      ₽{getTotalSales().toLocaleString('ru-RU')}
                    </Typography>
            ...
            <Grid item xs={12} md={4}>
              <Card>
                ...
                <CardContent>
                  <Typography variant="h4">
                    {getAverageOrdersPerDay().toFixed(1)}
                  </Typography>
                </CardContent>
              </Card>
            </Grid>
                  ...
                  {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      ...
                      >
                        ...
                        <YAxis />
                        <Tooltip 
                          formatter={(value: number) => [`₽${value.toLocaleString('ru-RU')}`, t('analytics.sales')]}
        ...
        <Box>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Typography variant="body1">
              {t('analytics.forecastExplanationText3')}
            </Typography>
          </Box>
        </Box>
      )}
                    ...
                    <TableContainer>
                      <Table>
                        <TableHead>
                          <TableRow>
                            <TableCell>{t('inventory.name')}</TableCell>
                            <TableCell align="right">{t('analytics.quantitySold')}</TableCell>
                        ...
                        <TableBody>
                          {topProducts.slice(0, 5).map((product) => (
                            <TableRow key={product.product_id}>
                              ...
                              <TableCell align="right">₽{product.revenue.toLocaleString('ru-RU')}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
