...
const ForecastChart: React.FC<ForecastChartProps> = ({ data, productName, loading }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <Card className="forecast-chart-card">
        <Empty description="Нет данных для прогноза" image={Empty.PRESENTED_IMAGE_SIMPLE} />
      </Card>
    );
  }
  ...
  return (
    <Card className="forecast-chart-card">
      <Title level={4} className="forecast-title">
        {productName ? `Прогноз запасов: ${productName}` : 'Прогноз запасов'}
      </Title>
      <Divider />
      ...
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <YAxis name="Количество" tick={{ fontSize: 12 }} domain={[0, 'dataMax + 5']} />
          <Tooltip
            contentStyle={{
